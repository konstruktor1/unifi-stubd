...
func parseRuntimeFlags(defaults appconfig.Config) (runtimeFlags, map[string]bool) {
	flags := runtimeFlags{
		...
		lldpSource:         flag.String("lldp-source", defaults.LLDPSource, "passive LLDP source: off; lldpd is planned but not implemented"),
		...
		discoveryInterface: flag.String("discovery-interface", defaults.DiscoveryInterface, "optional local interface name used for UDP discovery sends"),
		managementVLAN:     flag.Int("management-vlan", defaults.ManagementVLAN, "optional management VLAN ID reported to UniFi; 0 leaves it unset"),
