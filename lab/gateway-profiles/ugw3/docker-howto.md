...
```sh
PROFILE=lab/gateway-profiles/ugw3
...
```sh
docker compose -f "$PROFILE/compose.yaml" up -d --build
```

Inspect logs:

```sh
docker compose -f "$PROFILE/compose.yaml" logs --tail 120 ugw3-qemu
```

Run the local management CLI inside the same chroot:

```sh
docker compose -f "$PROFILE/compose.yaml" exec ugw3-qemu \
...
```sh
docker compose -f "$PROFILE/compose.yaml" down
```
