#!/bin/bash
# Start the selected UCG-Fiber firmware processes inside the lab container.
set -euo pipefail

preload="${UNIFI_FW_SIM_PRELOAD:-/mock/libubnthal_redirect.so}"
log_dir="${UNIFI_FW_SIM_LOG_DIR:-/tmp}"
model="${UNIFI_FW_SIM_MODEL:-UCGF}"
mac="${UNIFI_FW_SIM_MAC:-02:15:6d:00:a6:a8}"
...
if [[ ! -S "$udapi_socket" ]]; then
    echo "warning: $udapi_socket is not present; continuing with partial UCG-Fiber simulation" >&2
