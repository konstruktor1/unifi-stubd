// Package device exposes payload rendering through the stable device API.
// Callers build payloads from profiles and ports without importing renderer
// internals directly.
package device
...
func BuildPayload(profile Profile, id Identity, ports []Port) ([]byte, error) {
	data, err := payloadpkg.BuildPayload(profile.PayloadOptions(), id, ports)
	if err != nil {
...
func ClonePortOverrides(overrides []PortOverride) []PortOverride {
	return payloadpkg.ClonePortOverrides(overrides)
}
...
func ValidatePortOverride(override PortOverride, portCount int) error {
	if err := payloadpkg.ValidatePortOverride(override, portCount); err != nil {
		return fmt.Errorf("validate port override: %w", err)
	}
	return nil
}
...
func NormalizePortOverride(override PortOverride) PortOverride {
	return payloadpkg.NormalizePortOverride(override)
}
