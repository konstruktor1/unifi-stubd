...
func TestLookupProfile(t *testing.T) {
	profile, ok := LookupProfile("us16p150")
	if !ok {
...
func TestLookupProfileByModel(t *testing.T) {
	profile, ok := LookupProfile("US8P60")
	if !ok {
		t.Fatal("profile not found")
	}
...
func TestTenGigProfile(t *testing.T) {
	profile, ok := LookupProfile("us16xg")
	if !ok {
		t.Fatal("profile not found")
	}
...
func TestAutoMACIsStableAndProfileSensitive(t *testing.T) {
	first := AutoMAC("host|us16p150")
	second := AutoMAC("host|us16p150")
	third := AutoMAC("host|us16xg")
