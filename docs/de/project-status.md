...
`unifi-stubd` hat zwei verwandte, aber getrennte Lab-Spuren:

- Der Go-Hauptdienst ist ein sicherer UniFi-Device-Stub fuer isolierte Lab-
  oder Management-Netze. Er sendet Discovery- und Inform-Traffic, speichert
  Adoption-State und darf einen Controller den Host nicht blind provisionieren
  lassen.
- `lab/gateway-profiles/` enthaelt Firmware-Research-Profile. Diese Profile
  sind kein Produktcode und machen `unifi-stubd` nicht zu einem vollwertigen
  Gateway; sie sind kontrollierte Lab-Referenzen, um das Verhalten echter
  Firmware zu verstehen.

## Hauptdienst

Der Hauptdienst bietet aktuell:

- Discovery- und Inform-Framing fuer synthetische UniFi-Device-Identitaeten.
- Eingebaute Switch-Profile und experimentelle Gateway-Identitaetsprofile.
- YAML-Konfiguration, OpenRC/systemd-Units und Paket-Builds.
- Stub-, Observe- und Packaging-Workflows in der Doku.
- Einen Adoption-SSH-Shim, der die minimale Advanced-Adoption-Form akzeptiert,
  ohne beliebige Controller-Shell-Kommandos auszufuehren.

Die Sicherheitsgrenze bleibt unveraendert: Controller-getriggerte Restarts,
Upgrades, Shell-Kommandos und Host-Netzwerkaenderungen muessen explizit,
lab-lokal und pruefbar bleiben.
