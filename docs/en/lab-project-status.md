...
This page tracks lab and firmware-reference work. It is intentionally separate
from [Project Status](project-status.md), which describes the `unifi-stubd`
daemon itself.

The lab work exists to understand real UniFi firmware behavior and controller
expectations. It is not product code and it does not turn `unifi-stubd` into a
full gateway or UniFi OS replacement.
