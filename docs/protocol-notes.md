...
Bei alten Tests fuehrte ein fehlendes oder zu kleines `uptime` in `mac_table` zu Controller-Problemen. Deshalb sollte jeder MAC-Tabelleneintrag ein plausibles `uptime` besitzen.
...
- `US16XG`: 16-Port-10G-Profil fuer Aggregation-/SFP+-Tests.

Gateway-Modelle wie `UGW3`, `UGW4` oder `UXG` erst spaeter testen.
